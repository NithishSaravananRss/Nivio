...
class WatchPartyServiceSupabase {
  ...
  Future<String?> createSession({String? preferredCode}) async {
    ...
    try {
      ...
      _emitError('Failed to create watch party: $error');
      await leaveSession();
      return null;
    }
  }
  ...
  Future<bool> joinSession(String code) async {
    ...
    try {
      ...
      _emitError('Failed to join watch party: $error');
      await leaveSession();
      return false;
    }
  }
  ...
  Future<void> sendReaction(String emoji) async {
    if (_channel == null || _sessionCode == null || emoji.trim().isEmpty) {
      return;
    }
  ...
  Future<void> _joinChannel(String code) async {
    ...
    _channel!.onBroadcast(
      event: 'reaction',
      callback: _handleReactionBroadcast,
    );
    ...
    _channel!.subscribe((status, error) async {
      debugPrint('[party] channel status=$status error=$error');
      if (status == RealtimeSubscribeStatus.subscribed) {
        try {
          ...
          if (!subscribedCompleter.isCompleted) {
            subscribedCompleter.completeError(error);
          }
        }
        return;
      }
  ...
  void _handlePlaybackBroadcast(Map<String, dynamic> payload) {
    ...
    try {
      ...
      _emitError('Failed to parse party playback state: $error');
    }
  }
  ...
  void _handleStateSnapshot(Map<String, dynamic> payload) {
    ...
    try {
      ...
      _emitError('Failed to apply watch party snapshot: $error');
    }
  }
  ...
  void _handleChatBroadcast(Map<String, dynamic> payload) {
    try {
      _chatController.add(WatchPartyChatMessage.fromJson(payload));
    } catch (error) {
      if (kDebugMode) debugPrint('Failed to parse chat message: $error');
    }
  }

  void _handleReactionBroadcast(Map<String, dynamic> payload) {
    try {
      _reactionController.add(WatchPartyReaction.fromJson(payload));
    } catch (error) {
      if (kDebugMode) debugPrint('Failed to parse reaction: $error');
    }
  }
